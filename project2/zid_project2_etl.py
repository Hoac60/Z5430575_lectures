...
def read_prc_csv(tic, start, end, prc_col='Adj Close'):
    ...
    lower_tic = tic.lower()
    csvfile = os.path.join(cfg.DATADIR, f"{lower_tic}_prc.csv")
...
def monthly_return_cal(prc):
    ...
    pre_month_price = monthly_price.shift(1)
    monthly_returns = (monthly_price / pre_month_price) - 1
    monthly_returns = monthly_returns.dropna()
    monthly_returns.index = monthly_returns.index.to_period(freq='M')

    entries_counts = prc.resample('M').count()
    entries_counts.index = entries_counts.index.to_period(freq='M')
    vaild_months = entries_counts[entries_counts >= 18].index

    monthly_returns = monthly_returns[monthly_returns.index.isin(vaild_months)]
...
def aj_ret_dict(tickers, start, end):
    ...
    for ticker in tickers:
        ...
        daily_returns[lower_ticker] = daily_return_cal(price_data)
        monthly_returns[lower_ticker] = monthly_return_cal(price_data)

    df_daily_returns = pd.DataFrame(daily_returns)
    df_monthly_returns = pd.DataFrame(monthly_returns)

    df_daily_returns.index.name = 'Date'
    df_monthly_returns.index.name = 'Year_month'

    ret_dict = {
        'Daily': df_daily_returns,
        'Monthly': df_monthly_returns
    }
